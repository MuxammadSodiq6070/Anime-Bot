...
def main_menu_keyboard(lang='uz') -> ReplyKeyboardMarkup:
    ...
    kb.row(KeyboardButton(text=t['search']))
    kb.row(KeyboardButton(text=t['premium']), KeyboardButton(text=t['profile']))
    kb.row(KeyboardButton(text=t['watchlist']), KeyboardButton(text=t['history']))
    kb.row(KeyboardButton(text=t['top']), KeyboardButton(text=t['contact']))
...
def anime_detail_keyboard(bot_username: str, anime_id: int,
                          in_watchlist: bool, lang='uz') -> InlineKeyboardMarkup:
    kb = InlineKeyboardBuilder()
    kb.row(InlineKeyboardButton(
        text="▶️ Tomosha qilish",
        url=f"https://t.me/{bot_username}?start={anime_id}"
    ))
    wl_text = "🔕 Ro'yxatdan chiqarish" if in_watchlist else "🔔 Kuzatuvga qo'shish"
    kb.row(
...
# ── Watch episode ─────────────────────────────────────────────────────────────
def episode_keyboard(anime_id: int, episode_number: int,
                     total_episodes: int, protect: bool) -> InlineKeyboardMarkup:
    kb = InlineKeyboardBuilder()
    nav = []
    if episode_number > 1:
        nav.append(InlineKeyboardButton(
            text="⬅️ Oldingi", callback_data=f"ep_{anime_id}_{episode_number-1}"))
    if episode_number < total_episodes:
        nav.append(InlineKeyboardButton(
            text="Keyingi ➡️", callback_data=f"ep_{anime_id}_{episode_number+1}"))
    if nav:
        kb.row(*nav)
...
def premium_keyboard() -> InlineKeyboardMarkup:
    kb = InlineKeyboardBuilder()
    kb.row(InlineKeyboardButton(text="💳 30 kun - 5,000 UZS", callback_data="buy_premium"))
    kb.row(InlineKeyboardButton(text="🔙 Ortga",              callback_data="back_main"))
    return kb.as_markup()
...
def admin_panel_keyboard() -> InlineKeyboardMarkup:
    kb = InlineKeyboardBuilder()
    kb.row(
        InlineKeyboardButton(text="🔧 Asosiy sozlamlar",  callback_data="main_settings"),
...
def edit_anime_fields_keyboard() -> InlineKeyboardMarkup:
    ...
    for text, cb in [
        ...
        ("🖼 Rasm","edit_field_image"),("💎 VIP holati","edit_field_is_vip"),
    ]:
